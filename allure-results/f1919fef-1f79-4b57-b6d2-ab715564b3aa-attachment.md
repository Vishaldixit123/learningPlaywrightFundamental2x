...
- Name: 05_MultiElement_Table/01_MultiElementHandle.spec.ts >> Multi Element Handle 
- Location: tests/05_MultiElement_Table/01_MultiElementHandle.spec.ts:3:5
            ...
            - region "Account navigation" [ref=e575]:
              ...
              - generic [ref=e577]:
                ...
                - link "Register ›" [ref=e579] [cursor=pointer]:
                  - /url: "#register"
                - link "Forgotten Password ›" [active] [ref=e580] [cursor=pointer]:
                  - /url: "#forgotten-password"
...
```ts
  ...
  2  | 
  3  | test('Multi Element Handle ', async ({ page }) => {
  4  |     await page.goto('https://app.thetestingacademy.com/playwright/multiple_element_filter');
  5  |     await page.waitForTimeout(2000);
  6  |     const heading: Locator = page.getByRole('heading', { name: 'Student Login' });
  7  |     await expect(heading).toBeVisible();
  8  |     const allLink: string[] = await page.locator('a.list-group-item').allInnerTexts();
  9  |     console.log(allLink.length)
  10 |     await page.waitForTimeout(2000);
  11 | 
  12 |     for (let link of allLink) {
  13 |         if (link === 'Forgotten Password') {
  14 |             await page.getByTestId('forgotten-password-link').click();
  15 |         }
