...
- Location: tests/06_Select_DropDown/02_selectMulti.spec.ts:21:5
...
> 13 |     await page.getByTestId(selector).fill(value);
     |                                      ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  14 |     let values: string[] = await page.locator('tta-rs__option').allInnerTexts();
  15 |     await page.locator('tta-rs__option').first().click();
  16 |     await page.keyboard.press('Escape')
  17 |     return values;
  18 | 
  19 | }
  20 | 
  21 | test('Select multiple value', async ({ page }) => {
  22 |     await page.setViewportSize({ width: 1850, height: 900 });
  23 |     await page.goto('https://app.thetestingacademy.com/playwright/tables/select-boxes');
  24 |     await selectMultiple(page, ['Playwright', 'JUnit'], 'rs-multi-input');
  25 |     let selectValues: string[] = await selectAsync(page, 'Pu', 'rs-async-menu');
  26 |     console.log(selectValues)
  27 | 
  28 | })
```