...
- Name: 08_Keyword_Hover_DropDown/02_RightClick.spec.ts >> Handle Nasted Frame
- Location: tests/08_Keyword_Hover_DropDown/02_RightClick.spec.ts:3:5
...
```ts
  ...
  6  | 
  7  |     await page.getByTestId('ctx-target').click({ button: "right" });
  8  |     await page.waitForTimeout(3000)
  9  | 
  10 |     let allMenu: string[] = await page.locator('#ctx-menu li button span:first-child').allInnerTexts();
  11 | 
  12 |     for (let menu of allMenu) {
  13 |         console.log(menu)
  14 |     }
> 15 |     await page.getByText('Copy', { exact: true }).click();
     |                                                   ^ Error: locator.click: Error: strict mode violation: getByText('Copy', { exact: true }) resolved to 2 elements:
  16 | 
  17 | })
```