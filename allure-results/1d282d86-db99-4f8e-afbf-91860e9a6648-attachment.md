...
```
Error: locator.click: Unexpected token "/" while parsing css selector ".//td[normalize-space()="Pune"]/ancestor::tr/td/input". Did you mean to CSS.escape it?
Call log:
  - waiting for //table/tbody[@id="employee-body"]/tr >> nth=0 >> .//td[normalize-space()="Pune"]/ancestor::tr/td/input
...
```ts
  ...
  6  | 
  7  |     const row = page.locator('//table/tbody[@id="employee-body"]/tr');
  8  |     const rowNumber = await row.count();
  9  | 
  10 |     console.log('Row count:', rowNumber);
  11 | 
  12 |     for (let i = 0; i < rowNumber; i++) {
  13 |         const cells = row.nth(i).locator('td');
  14 |         const cellCount = await cells.count();
  15 | 
  16 |         for (let j = 0; j < cellCount; j++) {
  17 |             const cellText = (await cells.nth(j).innerText()).trim();
  18 | 
  19 |             if (cellText === 'Pune') {
> 20 |                 await row.nth(i).locator('.//td[normalize-space()="Pune"]/ancestor::tr/td/input').click();
     |                                                                                                   ^ Error: locator.click: Unexpected token "/" while parsing css selector ".//td[normalize-space()="Pune"]/ancestor::tr/td/input". Did you mean to CSS.escape it?
  21 |                 console.log(`Clicked row ${i + 1} because it contains Pune`);
  22 |                 break;
  23 |             }
  24 |         }
  25 |     }
